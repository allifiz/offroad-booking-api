...
@section('title', $booking->booking_code)
...
<header class="mt-5 flex flex-col gap-4 rounded-2xl bg-slate-950 p-6 text-white sm:flex-row sm:items-end sm:justify-between">
    ...
    <div class="text-right"><p class="text-sm text-slate-400">Total</p><p class="text-2xl font-black">Rp{{ number_format((float) $booking->total_amount, 0, ',', '.') }}</p></div>
</header>
...
<section class="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
    <div class="space-y-6">
        ...
        <article class="rounded-2xl border bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between"><h2 class="font-bold">Assignments</h2><span class="text-sm text-slate-500">{{ $booking->driverAssignments->count() }} data</span></div>
            <div class="mt-4 space-y-3">
                @forelse ($booking->driverAssignments as $assignment)
                    <div class="flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div><p class="font-bold">{{ $assignment->driver?->name }}</p><p class="text-sm text-slate-500">{{ $assignment->vehicle?->name }} · {{ $assignment->status->value }} · kapasitas {{ $assignment->vehicle?->capacity }}</p></div>
                        @if ($assignment->status->value !== 'cancelled')
                            <form method="POST" action="{{ route('admin.bookings.assignments.cancel', [$booking, $assignment]) }}">@csrf @method('PATCH')<button class="rounded-lg bg-red-50 px-4 py-2 text-sm font-bold text-red-700">Batalkan</button></form>
                        @endif
                    </div>
                @empty
        ...
        <article class="rounded-2xl border bg-white p-6 shadow-sm">
            <div class="flex items-center justify-between"><div><h2 class="font-bold">Alokasi peserta</h2><p class="text-sm text-slate-500">Peserta hanya dapat dialokasikan ke assignment accepted.</p></div><span class="text-sm text-slate-500">{{ $booking->participants->count() }} peserta</span></div>
            <div class="mt-4 space-y-3">
                @forelse ($booking->participants as $participant)
                    <form method="POST" action="{{ route('admin.bookings.allocations.update', $booking) }}" class="grid gap-3 rounded-xl border p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
                        ...
                        <div><p class="font-bold">{{ $participant->name }}</p><p class="text-sm text-slate-500">{{ $participant->phone ?: 'Tanpa nomor telepon' }}</p></div>
                        <label class="text-xs font-semibold uppercase text-slate-500">Kendaraan
                            <select name="driver_assignment_id" class="mt-1 w-full rounded-lg border-slate-300">
                                ...
                                @foreach ($acceptedAssignments as $assignment)
                                    <option value="{{ $assignment->id }}" @selected($participant->vehicleAllocation?->driver_assignment_id === $assignment->id)>{{ $assignment->driver?->name }} — {{ $assignment->vehicle?->name }} ({{ $assignment->vehicle?->capacity }} kursi)</option>
                                @endforeach
                            </select>
                        </label>
    ...
    <aside class="space-y-6">
        <article class="rounded-2xl border bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.bookings.status', $booking) }}" class="mt-4 space-y-3">@csrf @method('PATCH')
                <select name="status" class="w-full rounded-lg border-slate-300">@foreach (\App\Enums\BookingStatus::cases() as $status)<option value="{{ $status->value }}" @selected($booking->status === $status)>{{ ucfirst($status->value) }}</option>@endforeach</select>
        ...
        <article class="rounded-2xl border bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.bookings.assignments.store', $booking) }}" class="mt-4 space-y-3">@csrf
                <select name="driver_id" class="w-full rounded-lg border-slate-300"><option value="">Pilih driver</option>@foreach ($drivers as $driver)<option value="{{ $driver->id }}">{{ $driver->name }}</option>@endforeach</select>
                <select name="vehicle_id" class="w-full rounded-lg border-slate-300"><option value="">Pilih kendaraan</option>@foreach ($drivers as $driver) @php($driverVehicles = $driver->driverProfile?->vehicles ?? collect()) @foreach ($driverVehicles as $vehicle)<option value="{{ $vehicle->id }}">{{ $driver->name }} — {{ $vehicle->name }} ({{ $vehicle->plate_number }})</option>@endforeach @endforeach</select>
